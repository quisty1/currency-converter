// api.js smoke tests: convert, unitRate, isCacheStale, mergeRates, codes
// run: npm test
...
// rates fixture vs USD (fiat + crypto)
const rates = {
  base: 'USD',
  ...
  rates: {
    ...
    JPY: 150,
    USD: 1,
    // 1 BTC = 100_000 USD → 1/100000 BTC per 1 USD
    BTC: 1 / 100_000,
    ETH: 1 / 4_000,
  },
  cryptoMeta: {
...
// direct, inverse, EUR→RUB cross rate, same currency
...
// crypto ↔ fiat and crypto↔crypto via USD
...
// TTL: fresh / older than CACHE_TTL_MS
...
// mergeRates: fiat + crypto; ticker collision does not overwrite fiat
...
// fiat-only cache: no meta → seed, not an empty list
